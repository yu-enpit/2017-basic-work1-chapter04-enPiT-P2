...
public class ResultActivity extends AppCompatActivity {
    ...
    public void saveData(int myHand, int comHand, int gameResult) {
        ...
        int winningStreakCount = pref.getInt("WINNING_GAME_STREAK_COUNT", 0);
        int lastComHand = pref.getInt("LAST_COMHAND", 0);
        int lastGameResult = pref.getInt("LAST_RESULT", -1);
        ...
        editor.putInt("LAST_ MY_HAND", myHand);
    ...
    private int getHand(){
        ...
        if(gameCount == 1) {
            if(gameResult == 2) {
                ...
                //前回の勝負が１回目で、コンピュータが買った場合、
                //相手が出した手に勝つ手を出す
                hand = (lastMyHand - 1 + 3) % 3;
            }
        } else if(winningStreakCount > 0) {
            if(beforeLastComHand == hand) {
                //同じ手で連勝した場合は手を変える
                while(lastComHand == hand) {
                    hand = (int) (Math.random() * 3);
                }
